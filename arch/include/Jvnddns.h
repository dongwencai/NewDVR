...
#define JVN_DDNS_PEOPLEFU		11   	//peoplefu 	DDNS 客户私有定制
...
*参    数：  	 [IN]	ddnsType  	DDNS类型
			[IN]	ddnsHost		本地域名（类型ddnsType为JVN_DDNS_PEOPLEFU时，该参数仅限为dyndns.com; peoplefu.com; techonecctv.com）
			[IN]	ddnsUser		用户名
			[IN]	ddnsPwd		密码
...
*返回值： 	0：成功
			< 0：失败，请检查传入参数
*其    他： 	调用后上传更新一次ip，后续定时检测ip是否变化，变化则再次更新至ddns服务器，
			否则程序退出，下次调用时有效
